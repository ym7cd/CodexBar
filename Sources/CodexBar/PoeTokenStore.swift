...
protocol PoeTokenStoring: Sendable {
    func loadToken() throws -> String?
    func storeToken(_ token: String?) throws
}

enum PoeTokenStoreError: LocalizedError {
...
struct KeychainPoeTokenStore: PoeTokenStoring {
    private static let log = CodexBarLog.logger(LogCategories.poeTokenStore)

    private let service = "com.steipete.CodexBar"
    private let account = "poe-api-token"
    ...
    func loadToken() throws -> String? {
           ...
           Date().timeIntervalSince(timestamp) < Self.cacheTTL
        {
            ...
            Self.log.debug("Using cached Poe token")
            return cached
        }
        Self.cacheLock.unlock()
        ...
        guard status == errSecSuccess else {
            ...
            throw PoeTokenStoreError.keychainStatus(status)
        }

        guard let data = result as? Data else {
            throw PoeTokenStoreError.invalidData
        }
    ...
    func storeToken(_ token: String?) throws {
        ...
        if updateStatus != errSecItemNotFound {
            ...
            throw PoeTokenStoreError.keychainStatus(updateStatus)
        }
        ...
        guard addStatus == errSecSuccess else {
            ...
            throw PoeTokenStoreError.keychainStatus(addStatus)
        }
